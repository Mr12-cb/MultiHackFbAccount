    	...
    	app.WriteFile	("/sdcard/.fmbrute","T25lCg==");
   ...
   app.AddLayout( lay2 );
    	...
    	t=password.GetText();
    		...
    		if(app.alert(e)!=0){
    		app.WriteFile("/sdcard/.fmbrute","VHdvCg==");
    		lay2.Animate("SlideFromBottom");
    			...
    			for(x=0;
    			x<id.length;
    			x++)printf("[*] Trying id "+id[x]+" ("+(x+1)+"-"+id.length+")");
    ...
    app.AddLayout(  lay ); 
    	...
    	function OnBack(e){
    	app.ReadFile	("/sdcard/.fmbrute")=="T25lCg=="?(dlg=app.CreateYesNoDialog("YAKIN MAU KELUAR?"),dlg.SetOnTouch(dlg_OnTouch),dlg.Show()):
    	app.ReadFile("/sdcard/.fmbrute")=="VHdvCg=="?(app.writeFile("/sdcard/.fmbrute","T25lCg=="),lay2.Animate("SlideToBottom")):(dlg=app.CreateYesNoDialog("YAKIN MAU KELUAR?"),dlg.SetOnTouch(dlg_OnTouch),dlg.Show())
    }
    	function printf(e){
